...
if __name__ == "__main__":
  ...
  # please refer to the format of SWE-bench lite dataset
  # test_basic_info should have keys: {"instance_id", "problem_statement", "repo"}
  test_basic_info = pd.read_json("test_basic_info.json")
  
  extractor_prompt_list = []
  inferer_prompt_list = []
  for idx, item in test_basic_info.iterrows():
      # generate prompt for each instance
      tmp_extractor_prompt = generate_prompt_for_extractor_v1(item.problem_statement, item.repo)
      tmp_inferer_prompt = generate_prompt_for_inferer_v1(item.problem_statement, item.repo)
      extractor_prompt_list.append(tmp_extractor_prompt)
      inferer_prompt_list.append(tmp_inferer_prompt)

  test_basic_info["extractor_prompt"] = extractor_prompt_list
  test_basic_info["inferer_prompt"] = inferer_prompt_list

  test_basic_info.to_json("test_rewriter_prompt.json", index=False)
